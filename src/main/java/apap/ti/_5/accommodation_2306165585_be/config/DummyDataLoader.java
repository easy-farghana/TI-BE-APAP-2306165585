...
public class DummyDataLoader {
    ...
    private void createDummyProperties() {
        ...
        for (int i = 0; i < 15; i++) {
            ...
            Property property = Property.builder()
                ...
                .totalRoom(random.nextInt(20) + 10) // 10-29 rooms
                .activeStatus(1)
                .income(random.nextInt(50000000) + 10000000) // 10M-60M
            ...
            createRoomTypesForProperty(property, i + 1);
        }
    }
    
    private void createRoomTypesForProperty(Property property, int propertyNumber) {
        String[] roomTypeNames = {
            "Standard Room", "Deluxe Room", "Suite", "Family Room", "Executive Room",
            "Single Room", "Double Room", "Twin Room", "Presidential Suite", "Penthouse"
        };
        ...
        for (int i = 0; i < numRoomTypes; i++) {
            String roomTypeName = roomTypeNames[random.nextInt(roomTypeNames.length)];
            int floor = random.nextInt(5) + 1; // 1-5 floors
            String roomTypeId = String.format("%03d-%s-%d-%d", propertyNumber, roomTypeName, floor, i + 1);
    ...
    private void createRoomsForRoomType(RoomType roomType, Property property, int floor) {
        int numRooms = random.nextInt(5) + 2; // 2-6 rooms per room type
        ...
        for (int i = 1; i <= numRooms; i++) {
            ...
            Room room = Room.builder()
                ...
                .availabilityStatus(random.nextInt(2)) // 0 or 1
                .activeRoom(1)
                .maintenanceStart(random.nextBoolean() ? LocalDateTime.now().minusDays(random.nextInt(30)) : null)
                .maintenanceEnd(random.nextBoolean() ? LocalDateTime.now().plusDays(random.nextInt(30)) : null)
                .listAccommodationBooking(new ArrayList<>())
                .build();
            
            // Save room first without bookings
            room = roomRepository.save(room);
            
            // Create some bookings for this room after saving
    ...
    private void createBookingsForRoom(Room room) {
        ...
        for (int i = 0; i < numBookings; i++) {
            String bookingId = "BK" + System.currentTimeMillis() + random.nextInt(1000);
